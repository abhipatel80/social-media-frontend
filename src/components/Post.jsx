...
const Post = ({ val }) => {
    ...
    return (
        <>
            <div className="md:w-[30rem] sm:w-[24rem] shadow-sm overflow-auto w-[20rem] flex max-w-screen-md mx-auto" key={val?._id}>
                <div className="bg-white rounded-lg mt-4">
                    <div className="flex items-center justify-between p-4">
                        <NavLink to={`/profile/${val?.userId._id}`}>
                            <div className="flex items-center">
                                <img src={`http://localhost:4000/${val.userId.userImage}`} alt="User Profile" className="w-10 h-10 rounded-full mr-4 img-cover" />
                                <h2 className="text-lg font-semibold ml-1">{val?.userName}</h2>
                            </div>
                        </NavLink>
                        {val.userId._id === id ? "" :
                            isFollow ?
                                <button onClick={() => unfollow(val.userId._id)} className="text-blue-500 font-semibold md:text-base text-sm">
                                    Unfollow
                                </button>
                                :
                                <button onClick={() => follow(val.userId._id)} className="text-blue-500 font-semibold md:text-base text-sm">
                                    Follow
                                </button>
                        }
                    </div>
                    <img src={`http://localhost:4000/${val.postImage}`} alt="Post" className="w-full img-cover rounded-lg" />
                        ...
                        <div className="flex flex-col">
                            <div className='flex'>
                                {isLike ?
                                    ...
                                    <button className="text-2xl mr-6" onClick={() => addlike(val._id)}>
                            ...
                            {isComment ?
                                <div className='mt-2 flex'>
                                    <input type="text" placeholder='Add a comment...' onChange={(e) => setcomment(e.target.value)} className='py-0.5 px-4 w-full border border-black rounded-md' />
                                    <button className='bg-blue-600 px-3 font-semibold text-sm rounded-md text-white ml-4' onClick={() => addcomment(val._id)}>Post</button>
                                </div>
                                : ""}
                        </div>
                        <div className="mt-2">
                    ...
                    <div className="px-4 mb-1">
                        <p>
                            <span className="font-semibold">{val?.userName}</span> {val?.caption}
                        </p>
                    </div>
                                ...
                                <div className="px-4 py-1" key={index}>
                                    <div className="mb-1">
                                        <p className="text-sm flex w-full">
                                            ...
                                            {data?.comment}
                                            {data?.userId === id ?
                                                <button onClick={() => delcomment(data._id, val._id)} className='ml-auto cursor-pointer hover:text-red-600'>
