...
const HomePost = () => {
  ...
  const { followersPost } = useSelector(state => state.post);
  ...
  return (
    <>
      <div className='flex items-center justify-center bg-gray-50 pt-20 h-screen'>
        <div className={`bg-white p-4 rounded shadow-sm ${followCondition ? "lg:w-[25rem]" : ""}`}>
          {followCondition ?
            <>
              <h2 className='text-xl font-semibold'>Follow your favourite person</h2>
              {allUsers && allUsers?.map((val) => {
                return (
                  <div key={val._id}>
                    <div className="flex items-center">
                      <NavLink to={`/profile/${val._id}`} className="flex m-3">
                        <img src={`http://localhost:4000/${val.userImage}`} alt="User Profile" className="w-10 h-10 rounded-full mr-4 img-cover" />
            ...
            <>
              <div className="flex">
                <button
                  className={`flex-1 text-center py-2 ${tabIndex === 0 ? 'border-b-2 border-blue-500' : 'border-b'
                    }`}
                ...
                {likeCondition ?
                  "" :
                  <button
                    className={`flex-1 text-center py-2 ${tabIndex === 1 ? 'border-b-2 border-blue-500' : 'border-b'
                      }`}
